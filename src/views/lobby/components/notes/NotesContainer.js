...
function NotesContainer() {
  ...
  useEffect(() => {
    ...
    function create_note() {
      let random_tip;
      if (check_if_already_seen_rules()) {
        random_tip = instancesNames.date_tips();
  ...
  const check_if_already_seen_rules = () => {
    let already_seen_rules = JSON.parse(
      localStorage.getItem("already_seen_rules")
    );
    let last_seen = new Date(already_seen_rules);
    let todayDate = new Date();
    let res =
      last_seen?.setHours(0, 0, 0, 0) === todayDate.setHours(0, 0, 0, 0);
    if (res) return true;
    else {
